            ...
            <table class="table table-striped table-hover" id="logbooks">
                @if (!$today_logbook->isEmpty())
                    <button type="" class="btn btn-success m-3 btn-sm" data-bs-toggle="modal"
                        data-bs-target="#qrModal" id="buttonQR">Escanear QR
                    </button>
                @endif
                        ...
                        <tr>
                            ...
                            @isset($logbook->booking->assignment->assignment_name)
                                <td style="word-wrap: break-word; word-break: break-all; white-space: normal;">
                                    {{ $logbook->booking->assignment->assignment_name }}
                                </td>
                            @endisset
                            @isset($logbook->booking->event->event_name)
                                <td style="word-wrap: break-word; word-break: break-all; white-space: normal;">
                                    {{ $logbook->booking->event->event_name }}
                                </td>
                            @endisset
                            ...
                            @if ($logbook->check_in != null)
                                <td style="border-left: 1px dotted #0000006e; text-align:center;">
                                    @if ($logbook->check_out == null)
                                        {{-- boton check out --}}
                                        <button title="Registrar salida" class="btn btn-outline-secondary btn-sm d-block"
                                    ...
                                    @isset($logbook->commentary)
                                        <button title="Editar observaciones" class="btn btn-outline-secondary btn-sm d-block"
                            ...
                            @else
                                ...
                                <td style="border-left: 1px dotted #0000006e;">
                                    <button title="Ingresar con clave"
                                        class="btn btn-sm btn-outline-secondary d-block m-auto" data-bs-toggle="modal"
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body text-center">
                                        <form method="POST" action="{{ route('logbooks.update', $logbook->id) }}">
                                            @csrf
                                            @method('patch')
                        ...
                        <div class="modal fade" id="tokenModal{{ $logbook->id }}" tabindex="-1"
                            ...
                            <div class="modal-dialog modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <form action="{{ route('logbook.check') }}" method="post">
                                            @csrf
                                            <div class="form-group m-4">
                                                <input class="form-control form-control-lg" type="text" minlength="6"
                                                    style="font-size: 40px; text-align:center" name="token">
                                                <input type="hidden" name="logbook_id" value="{{ $logbook->id }}">
                                            </div>
                                            <div class="form-group m-4">
                                                <button type="submit" class="btn btn-sm btn-success w-100">
