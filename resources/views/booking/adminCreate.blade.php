    ...
    <div class="container-fluid">
        ...
        @isset($petition)
            <div class="alert alert-warning">
                <h4>Datos de petición</h4>
                <ul>
                    <li>Dia: {{ $petition->days }}</li>
                    <li>Hora de inicio: {{ $petition->start_time }}</li>
                    <li>Hora de fin: {{ $petition->finish_time }}</li>
                    <li>Mensaje: {{ $petition->message }}</li>
                </ul>
            </div>
        @endisset
        <div class="row">
            <div class="d-flex justify-content-center">
                <form id="formAdminCreate" class="createEventAssignment w-50" method="POST"
                    ...
                    <div class="mb-3">
                        ...
                        @isset($petition)
                            ...
                            @else
                                ...
                                @isset($petition)
                                    <option id="typeAssignment" value="assignment" selected disabled>Reserva
                                        Materia
                                    </option>
                                @else
                        ...
                        <div class="row">
                            ...
                            <div class="mb-3 col-md-6 col-sm-12">
                                <label for="cantParticipants" class="form-label">Cantidad de participantes <span
                                        class="text-secondary">(Paso 5)</span>
                                    <button type="button" class="btn btn-sm btn-light" data-toggle="tooltip"
                                        data-placement="top"
                                        title="Si la reserva a realizar tiene más participantes de los permitidos, debe realizar 2 o más reservas. Ej: 120 participantes son 2 reservas de 60 personas.">
                    ...
                    <div class="massiveEvent d-none">
                        ...
                        @isset($petition)
                        @else
                            {{-- profesores evento masivo --}}
                            <div class="mb-3">
                                <label for="nameTeacher" class="form-label">Profesor/a <span class="text-secondary">(Paso
                                        3)</span></label>
                                <select name="user_id" class="form-select select2-teacher" aria-label="Profesor/a"
                                    style="width: 100%;">
                                    <option value="-1" disabled></option>
                        ...
                        <div class="row">
                            ...
                            {{-- tipo de aula evento masivo --}}
                            <div class="mb-3 col-md-6 col-sm-12">
                                <label for="type" class="form-label">Tipo de aula <span class="text-secondary">(Paso
                                        5)</span></label>
                                <select name="classroom_type" class="form-select classroom_typeMassive">
                                    <option value="-1" disabled="" selected="">Seleccione un tipo de aula
                                    </option>
                                    <option value="Laboratorio">Laboratorio</option>
                                    <option value="Aula común">Aula común</option>
                            ...
                            <div class="mb-3 col-md-6 col-sm-12">
                                <label for="cantParticipants" class="form-label">Cantidad de participantes <span
                                        class="text-secondary">(Paso 6)</span>
                                    <button type="button" class="btn btn-sm btn-light" data-toggle="tooltip"
                                        data-placement="top"
                                        title="Si la reserva a realizar tiene más participantes de los permitidos, debe realizar 2 o más reservas. Ej: 120 participantes son 2 reservas de 60 personas.">
                                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.participantsMassiveEvent').on('change', function() {
                ...
                var tipoAula = $('.classroom_typeMassive').find('option:selected').val();
                console.log(tipoAula);
                $.ajax({
                    headers: {
        ...
        // Cambiar tipo de aula en materia
        ...
        // Cambiar tipo de aula en evento masivo
        $('.classroom_typeMassive').on('change', function() {
            // resetear campos de 'datos de reserva'
        ...
        // limpiar datos al cambiar fecha evento masivo
        $('#event_date').on('change', function() {
            $('.classroom_typeMassive option:selected').prop('selected', false);
            $('.classroom_typeMassive option:first').prop('selected', 'selected');
            $('.participantsMassiveEvent').val('');
            $('.classroomsMassiveEvent').empty();
            $('.start_timeMassiveEvent').empty();
            $('.finish_timeMassiveEvent').empty();
        ...
        // limpiar datos al cambiar fecha inicio de materia
        $('.start_date').on('change', function() {
            $('.finish_date').val('');
            $('.classroom_type option:selected').prop('selected', false);
            $('.classroom_type option:first').prop('selected', 'selected');
            $('.days option:selected').prop('selected', false);
            $('.days option:first').prop('selected', 'selected');
            $('.participants').val('');
            $('.classrooms').empty();
        ...
        // limpiar datos al cambiar fecha fin de materia
        ...
        $('#createBooking').on('click', function(e) {
            $('#formAdminCreate').submit(function() {
                $('input, select, #typeAssignment').attr('disabled', false);
